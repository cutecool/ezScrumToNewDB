...
import ntut.csie.ezScrum.test.CreateData.InitialPSQL;
...
public class TaskDAOTest {
	private PostgreSQLControl mPSQLControl = null;
	private MySQLControl mMySQLControl = null;
	...
	public void setUp() throws Exception {
		...
		ini.exe();
		new InitialPSQL(mConfig);

		mCP = new CreateProject(mProjectCount);
		mCP.exeCreate();

		mMySQLControl = new MySQLControl(mConfig);
		mMySQLControl.connect();
		
		mPSQLControl = new PostgreSQLControl(mConfig);
	...
	public void testDelete() throws SQLException {
		...
		valueSet.addEqualCondition(TaskEnum.PROJECT_ID, sProjectId);
		String query = valueSet.getSelectQuery();
		ResultSet resultSet = mPSQLControl.executeQuery(query);
	...
	public void testGetPartnersId_withOnePartner() {
		...
		mMySQLControl.executeInsert(addPartnerQuery);
	...
	public void testGetPartnersId_withTwoPartners() {
		...
		// execute add partner query
		mMySQLControl.executeInsert(addFirstPartnerQuery);
		mMySQLControl.executeInsert(addSecondPartnerQuery);
		// after add partner testGetPartnersId
		partnersId.clear();
		partnersId = TaskDAO.getInstance().getPartnersId(TEST_TASK_ID);
	...
	public void testAddPartner() throws SQLException {
		...
		ResultSet result = mMySQLControl.executeQuery(getPartnersIdQuery);
		while (result.next()) {
		...
		// execute get partners id query
		result = mMySQLControl.executeQuery(getPartnersIdQuery);
		try {
			while (result.next()) {
	...
	public void testRemovePartner() throws SQLException {
		...
		long id = mMySQLControl.executeInsert(query);
		...
		result = mMySQLControl.executeQuery(query);
		size = 0;
		while (result.next()) {
			size++;
		}
	...
	public void testPartnerExists() {
		...
		valueSet.addInsertValue(IssuePartnerRelationEnum.ISSUE_TYPE,
				IssueTypeEnum.TYPE_TASK);
		String query = valueSet.getInsertQuery();
		mMySQLControl.executeInsert(query);
